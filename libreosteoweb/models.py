...
class Patient(models.Model):
    ...
    address_complement = models.CharField(
        _("Address complement"), max_length=500, blank=True
    )
    ...
    job = models.TextField(_("Job"), blank=True, default="")
    ...
    laterality = models.CharField(
        _("Laterality"),
        max_length=1,
        choices=(("L", _("Left-handed")), ("R", _("Right-handed"))),
        blank=True,
        null=True,
    )
    ...
    creation_date = models.DateField(
        _("Creation date"), blank=True, null=True, editable=False
    )
...
InvoiceStatus = enum(
    "InvoiceStatus", "DRAFT", "WAITING_FOR_PAIEMENT", "INVOICED_PAID", "CANCELED"
)
...
class ExaminationComment(models.Model):
    ...
    examination = models.ForeignKey(
        Examination, verbose_name=_("Examination"), on_delete=models.PROTECT
    )
...
class Invoice(models.Model):
    ...
    professional_id_label = models.TextField(
        _("Professional id label"), default="Adeli"
    )
    location = models.TextField(_("Location"))
    number = models.TextField(_("Number"))
    patient_family_name = models.CharField(_("Family name"), max_length=200)
    patient_original_name = models.CharField(
        _("Original name"), max_length=200, blank=True
    )
    ...
    patient_address_complement = models.CharField(
        _("Address complement"), max_length=500, blank=True
    )
    ...
    office_identifier_label = models.TextField(
        _("Office identifier label"), default="SIRET"
    )
    office_address_street = models.CharField(
        _("Street"), max_length=500, blank=True, default=""
    )
    office_address_complement = models.CharField(
        _("Address complement"), max_length=500, blank=True, default=""
    )
    office_address_zipcode = models.CharField(
        _("Zipcode"), max_length=200, blank=True, default=""
    )
    office_address_city = models.CharField(
        _("City"), max_length=200, blank=True, default=""
    )
    ...
    replace = models.TextField(
        _("Invoice replaced"), blank=True, null=True, default=None
    )
    type = models.CharField(
        _("Invoice type"), max_length=10, blank=True, default="invoice"
    )
...
class OfficeSettings(models.Model):
    ...
    office_name = models.CharField(
        _("Office name"), max_length=250, blank=True, null=True
    )
    professional_id_label = models.CharField(
        _("Professional id label"),
        max_length=250,
        blank=False,
        null=False,
        default="Adeli",
    )
    invoice_office_header = models.CharField(
        _("Invoice office header"), max_length=500, blank=True
    )
    ...
    office_identifier_label = models.CharField(
        _("Office identifier label"),
        max_length=50,
        blank=False,
        null=False,
        default="SIRET",
    ...
    invoice_prefix_sequence = models.CharField(
        _("Invoice prefix sequence"), blank=True, null=True, max_length=3
    )
...
class TherapeutSettings(models.Model):
    ...
    office_identifier = models.CharField(
        _("Office identifier"), max_length=50, blank=True, null=True
    )
    ...
    MODULES_FIELDS = [
        ...
        },
        {
            "name": _("Examination"),
            "modules": [
                {"field": spheres_enabled, "image": "images/examination-spheres.png"},
                {
...
class FileImport(models.Model):
    ...
    status = models.IntegerField(
        _("validity status"), blank=True, default=None, null=True
    )
    analyze = None
    ...
    def delete(self, *args, **kwargs):
        ...
        if bool(self.file_patient):
            storage_patient, path_patient = (
                self.file_patient.storage,
                self.file_patient.path,
            )
        if bool(self.file_examination):
            storage_examination, path_examination = (
                self.file_examination.storage,
                self.file_examination.path,
            )
...
class Document(models.Model):
    ...
    document_date = models.DateField(
        _("Document date"), blank=True, null=True, default=None
    )
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        verbose_name=_("User"),
    ...
    def delete(self, *args, **kwargs):
        ...
        storage_document, path_document = (
            self.document_file.storage,
...
class PatientDocument(models.Model):
    ...
    patient = models.ForeignKey(
        Patient, verbose_name=_("patient"), on_delete=models.CASCADE
    )
    document = models.OneToOneField(
        Document, verbose_name=_("document"), on_delete=models.CASCADE, primary_key=True
    )
